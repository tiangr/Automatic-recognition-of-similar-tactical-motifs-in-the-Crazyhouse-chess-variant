...
# ---------------------------------------------------------------------------
# Model Feature Alignment Layer
# ---------------------------------------------------------------------------

def _all_raw_feature_names() -> list[str]:
    """Generates the full ordered list of all 65 engineered features."""
    probe = pair_features(
...
def load_active_feature_names() -> list[str]:
    """
    Looks for the Iteration 6 batch2+3 pruned model metadata JSON file to enforce
    the exact 47 features selected by RFECV. Falls back to all 65 features if 
    the metadata is missing.
    """
    meta_filename = "retrieval_model.json"
    
    # Common search locations (local directory, models subfolder, etc.)
    search_paths = [
        Path(meta_filename),
        Path("models") / meta_filename,
        Path("../data/models") / meta_filename,
        Path("outbatch3") / meta_filename,
    ]
    ...
    # Fallback to the full 65 features if companion JSON is not available
    return _all_raw_feature_names()


# Expose the stable, tailored feature-name ordering for downstream consumers
FEATURE_NAMES = load_active_feature_names()